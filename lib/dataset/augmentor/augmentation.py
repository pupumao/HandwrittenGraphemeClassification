...
def Fill_img(img_raw,target_height,target_width,label=None):
    ...
    if raw_width / raw_height >= target_width / target_height:
        ...
        img_fill = np.zeros(shape_need, dtype=img_raw.dtype)+np.array(0 ,dtype=img_raw.dtype)
...
def Random_crop(src,shrink):
    ...
    start_h=random.randint(0,int(h_shrink*1.5))
    start_w=random.randint(0,int(w_shrink*1.5))

    end_h = -random.randint(1,int(h_shrink*1.5))
    end_w = -random.randint(1,int(w_shrink*1.5))


    if (start_h-end_h)<64 or (start_w-end_w)<64:
        return src

    target_img=bimg[start_h:end_h,start_w:end_w,:]
